...
def base():
    ...
    context.update(critical_findings=[], high_uncertainty=False, unresolved_conflicts=[], unresolved_questions=[], human_signoff=True)
    return context
...
SCENARIOS = [
    ...
    ("critical_unacknowledged", {"critical_findings": ["critical"], "critical_result_acknowledged": False}, "BLOCKED"),
    ("uncertain_no_second_read", {"high_uncertainty": True, "second_read_complete": False}, "BLOCKED"),
...
def main():
    ...
    for name, changes, expected in SCENARIOS:
        context = base(); context.update(changes)
